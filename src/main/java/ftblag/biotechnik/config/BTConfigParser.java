...
public class BTConfigParser {
    ...
    private static void createDefault(File file) throws IOException {
        ...
        cfg.maxAmount = 1500;
        cfg.radius = 2;
        cfg.extractorToggleItem = Items.NETHER_STAR.getRegistryName().toString();
    ...
    public static boolean hasSpecificDrop(Entity entity) {
        if (entity == null || entity.getType() == null)
            return false;
        return cfg.specificDrops.containsKey(ForgeRegistries.ENTITIES.getKey(entity.getType()).toString());
    }

    public static int getSpecificDrop(Entity entity) {
        return cfg.specificDrops.get(ForgeRegistries.ENTITIES.getKey(entity.getType()).toString());
    }
