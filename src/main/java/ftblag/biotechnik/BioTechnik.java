...
public class BioTechnik {
    ...
    public static TileEntityType<TileEntityRFExtractor> extractorType = TileEntityType.Builder.of(TileEntityRFExtractor::new, extractor).build(null);
    public static ItemRFCollector collector = new ItemRFCollector(new Item.Properties().stacksTo(1).tab(BioTechnik.group));
    public static EntityType<EntityRFOrb> orbType = EntityType.Builder.<EntityRFOrb>of(EntityRFOrb::new, EntityClassification.MISC).setCustomClientFactory(EntityRFOrb::new).sized(0.125F, 0.125F).build(MODID + ":rforb");
